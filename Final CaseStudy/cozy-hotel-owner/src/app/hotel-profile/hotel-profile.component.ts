...
export class HotelProfileComponent implements OnInit {
  ...
  if (!token || !ownerId) {
    ...
    const apiUrl = `https://localhost:44340/api/Hotel/owner/${ownerId}`;
...
saveHotel() {
  ...
  this.http.put(`https://localhost:44340/api/Hotel/${this.hotel.id}`, this.hotel, { headers })
    .subscribe({
      next: () => {
...
deleteHotel() {
  ...
  this.http.delete(`https://localhost:44340/api/Hotel/${this.hotel.id}`, { headers })
    .subscribe({
      next: () => {
