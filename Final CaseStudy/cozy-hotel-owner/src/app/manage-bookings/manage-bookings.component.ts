...
export class ManageBookingsComponent implements OnInit {
  ...
  fetchBookings(): void {
    ...
    this.http.get<any[]>(`https://localhost:44340/api/Booking`)
      .subscribe({
        next: bookings => {
          // Filter by hotelId
  ...
  viewDetails(booking: any): void {
    ...
    this.http.get(`https://localhost:44340/api/User/${booking.userId}`, { headers })
      .subscribe({
        next: user => {
...
cancelBooking(bookingId: number): void {
  ...
  this.http.put(`https://localhost:44340/api/Booking/${bookingId}`, update, { headers }).subscribe({
    next: () => {
