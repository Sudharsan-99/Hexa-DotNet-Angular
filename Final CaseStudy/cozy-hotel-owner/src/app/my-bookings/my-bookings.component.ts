...
export class MyBookingsComponent implements OnInit {
  ...
  this.http.get<any[]>(`https://localhost:44340/api/Booking/user/${this.userId}`, { headers }).subscribe({
    next: (res) => {
      this.bookings = res;
      ...
      this.bookings.forEach((booking, i) => {
        if (!booking.hotel && booking.hotelId) {
          this.http.get<any>(`https://localhost:44340/api/Hotel/${booking.hotelId}`, { headers }).subscribe({
  ...
  cancelBooking(id: number) {
    ...
    const headers = new HttpHeaders().set('Authorization', `Bearer ${this.token}`);
    this.http.delete(`https://localhost:44340/api/Booking/${id}`, { headers }).subscribe({
      next: () => {
  ...
  reviewBooking(booking: any) {
    ...
    this.http.post('https://localhost:44340/api/Review', review, { headers }).subscribe({
